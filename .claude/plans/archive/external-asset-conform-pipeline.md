...
- **DR6 (hero-fidelity knobs — saturation guard + body sizing):** The 32px/typeclass-band conform crushed premium external art into a mangled ~26px blob (book fused into the face), defeating Track 2's whole purpose ("artist-quality = wow" per `art-pipeline-uplift.md`). Root cause of the fuse: `_material_for` matched by hue with no saturation guard, so near-white book pages (sat≈0.03, undefined hue→0) landed on the same `peach` ramp as the skin. Added three **mapping-driven** knobs (no game-coupling — the later modular Part formalizes the seam): (1) a `neutral` material + `neutral_sat_max` so desaturated pixels route to a neutral ramp instead of a chromatic one; (2) `body_fill` (fraction) / `body_height` (px) decoupling visual size from the gameplay typeclass band, so a hero fills a tall canvas. Wizard re-conformed at `48x64` + `body_fill 0.85` + `mauve_grey` neutral → body 53px (was 26), book legible, `PASS [external]`. TDD: `test_desaturated_pixel_routes_to_neutral`, `test_body_fill_override_beats_typeclass_band`. **Verified the art-direction spec is current intent** (`art-direction.md` draft 2026-06-11 + `art-pipeline-uplift.md` 2026-06-13 diagnoses the "uninspired procedural" complaint and motivates Track 2) — not stale instructions. **Known remaining artifact (candidate next pass):** per-cell bbox scaling makes the wizard pulse size in attack frames whose magic-circle inflates the bbox; fix is per-animation consistent scaling.
