...
description: >-
  ...
  warfare; TFT-style augment drafting over Battle Cats-style lane combat, premium meta
...
War-style in-battle Ascension that climaxes in unleashing your dragon, with TFT-style
augment drafting making every battle a fresh build puzzle. Premium, no IAP; campaign +
...
2. **One legible verb-set, deep matchup math.** Player verbs are exactly: deploy units,
   build/swap Conduits, aim Dragon's Breath, pick augments, fire the ultimate. Depth
...
3. **Randomness excites, never robs.** Every RNG surface is bounded, published in UI,
   and symmetric in PvP (shared augment tier paths, parity rules, pity).
...
- **Battle (~10 min, 3 acts):** Tier-I skirmish + economy investment → hard-pause
  augment drafts shape the build → Ascension thresholds unlock Tiers II/III → Dragon
...
|---|---|---|
| Deterministic battle sim (30 tps) | Design-Pending | Pure C# Logic-domain sim; frame-quantized combat (foreswing/backswing, KB counts, i-frames); seeded JmoRng |
| Units & type classes | Design-Pending | Melee/Ranged/Sniper/Aerial/Siege/Support; Tier I-III + Dragon; stat-shape-driven roles |
| Elements & synergies | Design-Pending | 5 elements; per-unit counter abilities (Strong 1.5x / Massive 3x / Resistant 0.25x); fielded-type thresholds 2/4 with dragon capstones |
| In-battle economy | Design-Pending | 12 mana/s drip, 300 cap, kill bounties, final-2:00 double drip |
| Conduits | Design-Pending | 3→5 Dragonspire sockets, ONE per conduit type; 6 buildable/sellable structures; the resource-management identity |
| Ascension | Design-Pending | Trickle + lane-control meter (kills capped at 30%/threshold); Tier II/III/Dragon; +25% drip per threshold; dragons via visible Summoning escrow |
| Draconic Augments | Design-Pending | 1-of-3 hard-pause drafts (~2:00/5:00/8:00, per-level authorable); Silver/Gold/Prismatic tier paths published at battle start; PvP split blind draft |
| Dragon's Breath + Wrath | Design-Pending | Manually aimed elemental base attack (anti-air verb) + 60s comeback ultimate |
| Campaign (Cinderfell Marches) | Design-Pending | 15 levels → boss dragon Pyraxis; mirrored-pool AI personas (Rusher/Powerhouse/Streamer); magnification scaling |
| Local PvP | Design-Pending | KB+mouse vs gamepad; War Standard level clamp; shared augment tier path |
| Meta progression | Design-Pending | Gold levels / Sigil unlocks; Dragon Rank caps; Trophies; Awakening; Heat ladder; RNG-control unlocks |
| Daily Skirmish | Design-Pending | Date-seeded, first-attempt-scores, local leaderboard |
| Art pipeline | Design-Pending | 640x360 integer-scaled pixel art, Resurrect 64 palette, 6 parametric TypeClass templates (Python/PIL) + manifest-driven SpriteFrames import; sim owns combat timing |
...
levels + gold levels. **v0.2** = augments + local PvP + synergies. **v1.0** = full
