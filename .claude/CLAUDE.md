...
Source of truth for design, lore, formulas, Jmodot framework docs. Trigger: lore/formulas/design rules/todos/framework research. The vault is a normal filesystem path (`C:\Users\jmund\OneDrive\Documents\ObsidianVault\DevProjects\DraconicWars\`, and `...\Jmodot\`) — **native `Read`/`Write`/`Edit`/`Grep`/`Glob` are the default** (confirmed safe even on docs open in the Obsidian app). Full conventions: `obsidian_conventions` skill (auto-loads).
*   **Read:** ONLY within `DevProjects/DraconicWars` or `DevProjects/Jmodot`. Synthesis-shaped reads still route to `read_files` (§9).
*   **Write:** project-specific docs → `DraconicWars/Claude/`; Jmodot library-general → `Jmodot/Claude/`. **Tiebreaker:** if a doc would be useful in another game built on Jmodot, it goes under `Jmodot/Claude/`.
...
**DraconicWars**: Godot 4.6 (2D — Godot Physics 2D; Jolt only configured for incidental 3D), C# (.NET 9.0), Jmodot framework. Concept: 2D lane-based auto-battler / tower-defense / resource-management hybrid — Battle Cats × Cartoon Wars lane combat with TFT-style draft/augment replayability; campaign + local PvP; dragons as the aspirational unit class.

### Domain Split (feeds Hybrid TDD above)
*Provisional until first architecture pass locks subsystem names — refine then.*
*   **Logic Domain (Strict TDD):** combat math (damage/DPS/counter resolution), in-battle economy (mana generation/costs), unit stat models + rarity/upgrade curves, wave/spawn scheduling, draft/augment offer logic (odds tables, bounded RNG), meta-progression data + save schema.
*   **Gameplay Domain (Integration + Inspection):** lane battlefield scenes, unit entity AI/HSM, deployment UI/HUD, VFX/animation, campaign flow/menus, local PvP input.
